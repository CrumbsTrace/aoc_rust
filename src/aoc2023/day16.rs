...
pub fn run(input: &str) -> (usize, usize) {
    let mirror_grid = input.trim().lines().map(|l| l.as_bytes()).collect_vec();
...
fn find_best_result(grid: &[&[u8]]) -> usize {
    ...
    edge_positions
        ...
        .map(|pos| {
            ...
            for dir in valid_dirs {
                ...
                let unique_positions = visited.iter().map(|(p, _)| *p).collect::<HashSet<_>>();
                if unique_positions.len() > best_count {
                    best_count = unique_positions.len();
                }
            }
            best_count
        })
...
fn traverse(
    ...
    pos: (usize, usize),
    dir: (i8, i8),
    visited: &mut HashSet<VisitedItem>,
) {
...
fn in_bounds(grid: &[&[u8]], pos: (i8, i8)) -> bool {
    pos.0 >= 0 && pos.0 < grid.len() as i8 && pos.1 >= 0 && pos.1 < grid[0].len() as i8
}

fn get_edge_positions(grid: &[&[u8]]) -> Vec<(usize, usize)> {
    let mut edge_positions = Vec::new();
    for x in 0..grid.len() {
        edge_positions.push((x, 0));
        edge_positions.push((x, grid[0].len() - 1));
    }
    for y in 0..grid[0].len() {
        edge_positions.push((0, y));
        edge_positions.push((grid.len() - 1, y));
    }
    edge_positions
}
